/*
 * Tokyojihatsu（~/Developer/Tokyojihatsu/scripts/release.ts, 2026-09-25 時点）から複製。
 * 同じ作者のコード。元のリポジトリは変更していない。
 */
 ...
 * 決めたときにだけ 1 つ上げる。Tokyojihatsu で 0.4.0〜0.4.2 が欠番になったのは、
 * 検証のたびに版を上げていたからだった。
 *
 * スクリーンショットも同じ理由でここで見る。**審査中は Store listing を
 * 編集できない**ので、画像が古いまま提出すると、審査が終わるまで直せない。
 * Tokyojihatsu 0.4.3 では About の文言を直したのに画像は 2 版ぶん古いままで、
 * ストアの画像がアプリの実装と矛盾したまま公開されていた。
 */
