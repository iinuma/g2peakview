...
`app:release` は「assets/screenshots がコードより後にコミットされているか」で見る。
撮り直した画像が前と**バイト単位で同じ**だと git に差分が出ず、確認したことが
残らない。そのときはここに 1 行足す。

| 日付 | コード | 結果 |
| --- | --- | --- |
| 2026-09-25 | `ce19309`（メニュー整理・文字を先に更新） | 3 枚とも撮り直し、前回と同一（主画面は変わっていない） |
